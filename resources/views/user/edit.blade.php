            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <form method="POST" action="{{ url('user/edit/'.$user->id) }}" autocomplete="off">
                            @csrf
                            <div class="form-group row">
                                <span for="name" class="col-md-4 text-md-right">Name <star class="star">*</star></span>
    
                                <div class="col-md-6">
                                    <input id="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ $user->name }}" placeholder="Name" autofocus>
                                     @if ($errors->has('name'))
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <input id="email" type="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ $user->email }}" placeholder="Email"> @if ($errors->has('email'))
                                    <span class="text-danger" role="alert">
                                       ...
                                       <div class="col-md-6">
                                            <select name="role" class="form-control" data-title="Single Select" data-style="btn-default btn-outline" data-menu-style="dropdown-blue">
                                                @foreach ($role as $item)
                                                    <option value="{{ $item->id}}" {{ $item->user_id == $user->id ? "selected" : ""}}> {{ $item->display_name}}</option>
                                                @endforeach
                                            </select>
                                            @if ($errors->has('role'))
                                                <span class="text-danger" role="alert">
                            ...
                            <div class="form-group row mb-0">
                                <div class="col-md-12">
                                    <a href="{{url('user')}}" ><input type="button" class="btn btn-danger pull-right" value="Cancel">
                                    </a>
